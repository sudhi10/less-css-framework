...
var Ruleset = function (selectors, rules, strictImports, visibilityInfo) {
    ...
    this.rules = rules;
    this._lookups = Object.create(null);
    ...
    this._iterator = [1, [-1, this.rules]];
    this._pos = this._iterator[1];
...
Ruleset.prototype.eval = function (context) {
    ...
    var rules = this.rules ? this.copyRules() : null,
    ...
    ruleset.firstRule();
    while ((rule = ruleset.nextRule())) {
        if (rule.evalFirst) {
            this._pos[1][this._pos[0]] = rule.eval(context);
        }
    }
    ...
    while ((rule = ruleset.nextRule())) {
        if (!rule.evalFirst) {
            this._pos[1][this._pos[0]] = rule = rule.eval ? rule.eval(context) : rule;
        }
    }
...
Ruleset.prototype.firstRule = function() {
...
Ruleset.prototype.nextRule = function() {
    var i = this._iterator,
        item, pos = i[i[0]];
    if (pos[1] === undefined) {
        return false;
...
// Copies and flattens any rule arrays via the iterator
Ruleset.prototype.copyRules = function() {
    var r, rules = [];
    this.firstRule();
    while ((r = this.nextRule())) {
        rules.push(r);
    }
    return rules;
};
